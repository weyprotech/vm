                    ...
                    <div class="widget-body no-padding">
                        <form id="data-form" class="form-horizontal" method="post" enctype="multipart/form-data">
                            ...
                            <div id="content" class="tab-content"><?php $i = 1; ?>
                                ...
                                <?php if ($this->langList): ?>
                                    <?php foreach ($this->langList as $lrow): ?>
                                        ...
                                        <div class="tab-pane" id="hb<?= $i++ ?>">
                                            <input type="hidden" name="langList[<?= $lrow->langId ?>][postId]" value="<?= $row->postId ?>">
                                            ...
                                            <fieldset>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-sm-6">
                                                        <input class="form-control" type="text" id="title-<?= $lrow->langId ?>" name="langList[<?= $lrow->langId ?>][title]" value="<?= @$langData->title ?>" required>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-sm-9">
                                                        ...
                                                        <input type="hidden" id="content" name="langList[<?= $lrow->langId ?>][content]">
                                                    </div>
                                                </div>
                                            </fieldset>
                                        </div>
                            ...
                            <div class="widget-footer">
                                ...
                                <button type="button" class="btn btn-default" onclick="location.href='<?= site_url("backend/brand/post/index/".$brandId . $this->query) ?>';">Return</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $Table = $('#dt_basic').DataTable({
            ...
            "ajax": {
                "url": "<?= site_url("backend/ajax/brand/post/get_post_img") ?>",  //傳到homepage slider
                "data": function (data) {
                    data.postid = '<?= $postId ?>';
                }
            },
    ...
    function imgUpload(Id)
    {
        ...
        swal({
            ...
            html:'<input type="file" id="transactionImg" multiple="multiple" onchange="change_img()" class="swal2-file transactionImg" placeholder="" style="display: block;width:92%"><br><strong>Note:</strong> Resolution is <strong>510 x 288</strong>. Format is JPG and PNG</strong><br><span stle="float:left">Youtube</span><input type="text" id="youtube" name="youtube" style="width:80%"></p>'
        }).then(function (e) {
            var data = new FormData();
            ...
            data.append('Id',Id);
            $.ajax({
                url:'<?= site_url("backend/ajax/brand/post/upload_img") ?>',
                type:'post',
                data:data,
                cache:false,
                dataType:'json',
    ...
    function delete_imgList(Id){
        swal({
            ...
            $.ajax({
                url:'<?= site_url("backend/ajax/brand/post/delete_img") ?>',
                type:'post',
                data:{Id : Id},
                dataType:'json',
                success:function(response){
