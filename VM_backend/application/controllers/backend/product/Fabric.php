...
class Fabric extends Backend_Controller
{
    ...
    {
        if (!$row = $this->product_model->get_product_fabric_by_pid($pId)):
            $id = $this->product_model->insert_product_fabric($pId);
            $row = $this->product_model->get_product_fabric_by_id($id); 
        endif;
        ...
        if ($post = $this->input->post(null, true)):
            ...
            if ($row->uuid != $post['uuid']):
                ...
                $this->product_model->update_product_fabric($row, $post);
                $this->set_active_status('success', 'Success');
            endif;

            redirect('backend/product/fabric/edit/' . $pId . $this->query);
        endif;
    ...
    {
        $content = $this->load->view('backend/product/fabric/' . $page, $data, true);
