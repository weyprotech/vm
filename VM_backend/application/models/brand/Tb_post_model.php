...
class Tb_post_model extends MY_Model
{
    ...
    public function get_post_select($filter = false, $order = false, $limit = false, $langId = false)
    {
        if ($langId):
            ...
            $this->db->join('tb_brand_post_lang as lang', 'lang.postId = post.postId AND lang.langId = ' . $langId, 'left');
        endif;
        $this->set_filter($filter);
        $this->set_order($order);
        $this->set_limit($limit);
        $query = $this->db->where('post.is_enable', 1)->get('tb_brand_post as post');
    ...
    public function count_post($filter = false, $langId = false)
    {
        ...
        $this->set_filter($filter);
        return $this->db->where('post.is_enable', 1)->count_all_results('tb_brand_post as post');
    }
    ...
    public function get_post_by_id($postId = false, $langId = false, $boolean = array('enable' => true, 'visible' => true))
    {
        ...
        $query = $this->db->where('post.postId', $postId)->get('tb_brand_post as post');
    ...
    {
        $post['order'] = $this->count_post(array(array('field' => 'post.brandId','value' => $post['brandId'])))+1;
        ...
        $this->db->insert('tb_brand_post', $insert);
    ...
    public function update_post($post, $update)
    {
        ...
        $this->db->update('tb_brand_post', $update, array('postId' => $post->postId));
        return true;
    }
    ...
    private function get_post_lang_select($filter = false)
    {
        $langList = array();
        $this->set_filter($filter);
        $query = $this->db->get('tb_brand_post_lang');
        if ($query->num_rows() > 0):
    ...
    {
        $this->db->where('postId', $postId)->update_batch('tb_brand_post_lang', $update, 'langId');
        ...
        if (!empty($insert)):
            ...
            return $this->db->insert_batch('tb_brand_post_lang', $insert);
        endif;
    ...
    public function get_post_img_select($filter = false,$order = false,$limit = false){
        ...
        $this->set_order($order);
        $this->set_limit($limit);
        $query = $this->db->get('tb_brand_post_img as post_img');
        if($query->num_rows() > 0){
            return $query->result();
        }
        return false;
    ...
    public function get_post_img_by_id($id){
        $query = $this->db->where('Id',$id)->get('tb_brand_post_img as post_img');
        if($query->num_rows() > 0){
            return $query->row();
        }
        return false;
    ...
    public function count_post_img($filter = false){
        $this->set_filter($filter);
        $count = $this->db->count_all_results('tb_brand_post_img as post_img');
        return $count;
    }
    ...
    public function insert_post_img($post){
        ...
        $this->db->insert('tb_brand_post_img',$insert);
    ...
    public function update_post_img_select($img,$post){
        $update = $post;
        $this->db->update('tb_brand_post_img',$update,array('Id' => $img->Id));
        return true;
    }
    ...
    public function delete_post_img($img){
        ...
        $this->db->delete('tb_brand_post_img',array('Id' => $img->Id));
    ...
    public function get_post_message_select($filter = false, $order = false, $limit = false, $langId = false)
    {
        if ($langId):
            $this->db->select('post.*, lang.title, lang.content');
            $this->db->join('tb_brand_post_lang as lang', 'lang.postId = post.postId AND lang.langId = ' . $langId, 'left');
            $this->db->join('tb_brand_post_message as message','message.postId = post.postid','left');
        endif;
        $this->set_filter($filter);
        $this->set_order($order);
        $this->set_limit($limit);
