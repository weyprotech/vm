...
class Tb_inspiration_message_model extends MY_Model
{
    ...
    public function update_inspiration_message($message, $post)
    {
        $update = $this->check_db_data($post);

        $this->update('tb_inspiration_message', $update, array('Id' => $message->Id));
        return true;
    }
    /******************** End brand Model ********************/
    ...
    {
        $this->db->select('message.*, member.first_name, member.last_name');
