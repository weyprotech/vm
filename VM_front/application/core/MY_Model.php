...
class MY_Model extends CI_Model
{
    ...
    protected function set_filter($filter)
    {
        if ($filter && $filter = $this->_check_filter_value($filter)) {
            ...
            foreach ($filter as $type => $row) {
                switch ($type) {
                    case ($type == 'whereIn'):
                        $this->db->where_in($row['field'], $this->db->escape_like_str($row['value']));
                        break;
                    case ($type == 'orWhere'):
                        $this->db->or_where($row['field'], $this->db->escape_like_str($row['value']));
                        break;
                    case ($type == 'like'):
                    ...
                    case ($type == 'orLike'):
                        ...
                        break;
                    case ($type == 'having'):
                        $this->db->having($row['field'], $this->db->escape_like_str($row['value']));
                        break;
                    case ($type == 'other'):
                    ...
                    case ($type == 'group'):
                        ...
                        break;
                    default:
                        $this->db->where($row['field'], $this->db->escape_like_str($row['value']));
                }
            }
            $this->db->group_end();
        }
    ...
    protected function set_default_filter($filter)
    {
        if ($filter && $filter = $this->_check_filter_value($filter)) {
            $this->db->group_start();
            foreach ($filter as $type => $row) {
