...
$(document).ready(function () {
  ...
  if ($('.dropdown_menu').length > 0) {
    $('.dropdown_menu').on(clickHandler, '.dropdown_head', function(event) {
      $(this).next('.dropdown_list').stop().slideToggle();
    });
    $('.dropdown_menu').on(clickHandler, '.dropdown_list a', function (event) {
      if ($(this).parents('.dropdown_menu').hasClass('dropdown_select')) {
        ...
        $(this).parents('.dropdown_list').prev('.dropdown_head').children('.sortText').text(sortText);
      }
      $(this).parents('.dropdown_list').stop().slideUp();
    });
    $(document).on(clickHandler, function (event) {
      if (!$(event.target).is('.dropdown_menu, .dropdown_menu *')) {
  ...
  if ($('.quantity_counter').length > 0) {
    $('.quantity_counter').on(clickHandler, '.plus', function(event) {
      ...
      $(this).siblings('.minus').attr('disabled', !a);
    });

    $('.quantity_counter .minus').attr('disabled', !$(this).siblings('quantity').val());
    $('.quantity_counter').on(clickHandler, '.minus', function (event) {
      var $qtde = $(this).siblings($('.quantity'));
      var b = $qtde.val();
    ...
    $(window).on('scroll', function () {
      ...
      if(windowWidth >= 1280) {
        if (($(window).scrollTop() + $(window).height() - $('#fixedButtons').outerHeight()) > ($(document).height() - $('#fixedButtons').outerHeight() - $('#footer').outerHeight())) {
          $('#fixedButtons').css({
            'position': 'absolute',
            'top': $(document).height() - $('#footer').outerHeight() - $('#fixedButtons').outerHeight() - 16,
            'bottom': 'auto'
          });
        } else {
          $('#fixedButtons').css({
