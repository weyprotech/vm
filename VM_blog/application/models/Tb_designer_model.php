...
class Tb_designer_model extends MY_Model
{
    ...
    public function insert_designer($post)
    {
        ...
        if (isset($_FILES['designiconImg']) && !$_FILES['designiconImg']['error']):
            $insert['designiconImg'] = $this->uploadFile('designicon', $post['designerId'] . '/', 360);
        endif;
        if (isset($_FILES['designImg']) && !$_FILES['designImg']['error']):
            $insert['designImg'] = $this->uploadFile('design', $post['designerId'] . '/', 360);
        endif;
    ...
    public function update_designer($designer, $post)
    {
        ...
        if (isset($_FILES['designiconImg']) && !$_FILES['designiconImg']['error']):
            $update['designiconImg'] = $this->uploadFile('designicon', $designer->designerId . '/', 360);
        endif;
        if (isset($_FILES['designImg']) && !$_FILES['designImg']['error']):
            $update['designImg'] = $this->uploadFile('design', $designer->designerId . '/', 360);
        endif;
        if (isset($_FILES['hometownpost1Img']) && !$_FILES['hometownpost1Img']['error']):
    ...
    private function set_designer_join($langId)
    {
        ...
        if ($langId):
            $this->db->select('lang.name,lang.personalImg,lang.country,lang.personal_title,lang.personal_content');
