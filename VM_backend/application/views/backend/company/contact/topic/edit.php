                        ...
                        <form id="data-form" class="form-horizontal" method="post" enctype="multipart/form-data"
                            ...
                            <div id="content" class="tab-content"><?php $i = 1; ?>
                                ...
                                <?php if ($this->langList): ?>
                                    <?php foreach ($this->langList as $lrow): ?>
                                        <div class="tab-pane" id="hb<?= $i++ ?>">
                                            <input type="hidden" name="langList[<?= $lrow->langId ?>][tId]" value="<?= $topicId ?>">
                                            <input type="hidden" name="langList[<?= $lrow->langId ?>][langId]" value="<?= $lrow->langId ?>">
                                            <fieldset>
                                                <legend><?= $lrow->name ?></legend>

                                                <div class="form-group">
                                                    <label class="col-sm-2 control-label" for="name-<?= $lrow->langId ?>">Topic</label>

                                                    <div class="col-sm-9">
                                                        <input class="form-control" type="text" id="name-<?= $lrow->langId ?>" name="langList[<?= $lrow->langId ?>][topic]" value="<?= $row->langList[$lrow->langId]->topic ?>" data-bv-notempty="true" data-bv-notempty-message=" ">
                                                    </div>
                                                </div>
                                            </fieldset>
                                        </div>
                            ...
                            <div class="widget-footer">
                                ...
                                <button type="button" class="btn btn-default" onclick="location.href='<?= site_url("backend/company/topic" . $this->query) ?>';">Return</button>
                            </div>
                        </form>
                    </div>
                </div>
