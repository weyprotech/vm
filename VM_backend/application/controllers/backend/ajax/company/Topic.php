...
class Topic extends Ajax_Controller
{
    ...
    public function get_topic_data($data = array())
    {
        ...
        $this->load->model('company/tb_topic_model', 'topic');        
        ...
        if ($topicList):
            foreach ($topicList as $row):
                $data[] = array(                    
                    ...
                    'topic' => $row->topic,
                    'action' => $this->get_button('edit', 'backend/company/topic/edit/' . $row->topicId . $query) . $this->get_button('delete', 'backend/company/topic/delete/' . $row->topicId . $query)
                );
            endforeach;
        endif;
