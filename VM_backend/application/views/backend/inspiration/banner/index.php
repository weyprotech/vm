                        ...
                        <form class="form-horizontal" method="post" enctype="multipart/form-data">
                            <div class="table-responsive">
                                <table id="dt_basic" class="table table-bordered table-striped text-center">
                                    <thead>
                                    <tr>
                                        <th width="30" class="text-center hidden-xs">Visibel</th>
                                        <th width="50%" class="text-center hidden-tablet hidden-md">Image</th>
                                        <th width="70" class="text-center hidden-tablet">Order</th>
...
<script>
    $(document).ready(function () {
        ...
        var $Table = $('#dt_basic').DataTable({
            ...
            "ordering": false,
            "serverSide": true,
            "sDom": "<'dt-toolbar'<'col-sm-8 hidden-xs' <'back_button input-group'>><'col-xs-12 col-sm-4'Tl>>" + "t" + "<'dt-toolbar-footer'<'col-sm-6 col-xs-12 hidden-xs'i><'col-xs-12 col-sm-6'p>>",
            "oTableTools": {
                "aButtons": [{
                    "sExtends": "text",
                    ...
                    "fnClick": function (nButton, oConfig, oFlash) {
                        $(nButton).parents('form:first').attr('action',  '<?= site_url('backend/inspiration/banner/save/'.$inspirationId) ?>').submit();
                    }
                }, {
                    "sExtends": "text",
                    ...
                    "fnInit": function (nButton, oConfig) {
                        $(nButton).css('margin-left', 5).css('text-shadow','0 -1px 0 rgba(0, 0, 0, 0.5), 0 1px 0 rgba(255, 255, 255, 0.3)').attr('href', '<?= site_url("backend/inspiration/banner/add/".$inspirationId) ?>');
                    }
                }]
            },
            "ajax": {
                "url": "<?= site_url("backend/ajax/inspiration/banner/get_banner_data") ?>",
                "data": function (data) {
