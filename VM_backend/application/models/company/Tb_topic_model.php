...
class Tb_topic_model extends MY_Model
{
    ...
    public function get_topic_select($filter = false, $order = false, $limit = false, $langId = false)
    {
        ...
        if($langId){
            ...
            $this->db->join('tb_contact_topic_lang as lang','lang.tId = topic.topicId AND lang.langId ='.$langId,'left');
        }
        $query = $this->db->where('topic.is_enable', 1)->get('tb_contact_topic as topic');
        if ($query->num_rows() > 0):
            $result = $query->result();
            if(!$langId){
                foreach($result as $i => $row){
                    $result->langList = $this->get_topic_lang_select(array(array('field' => 'tb_contact_topic_lang.tId','value' => $row->topicId)));
                }
            }
            return $result;
    ...
    {        
        ...
        return $this->db->where('topic.is_enable', 1)->count_all_results('tb_contact_topic as topic');
    }
    ...
    {        
        ...
        $query = $this->db->where('topic.topicId', $topicId)->get('tb_contact_topic as topic');
        
        if ($query->num_rows() > 0):
            $topic = $query->row();
            $topic->langList = $this->get_topic_lang_select(array(array('field' => 'tb_contact_topic_lang.tId','value' => $topic->topicId)));
    ...
    public function insert_topic($post)
    {
        ...
        $this->db->insert('tb_contact_topic', $insert);
        return true;
    }
    ...
    public function update_topic($topic, $post)
    {
        ...
        $this->db->update('tb_contact_topic', $update, array('topicId' => $topic->topicId));
        return true;
    }
    ...
    private function get_topic_lang_select($filter = false)
    {
        $langList = array();
        $this->set_filter($filter);
        $query = $this->db->get('tb_contact_topic_lang');
        if ($query->num_rows() > 0):
    ...
    {
        $this->db->where('tId', $topicId)->update_batch('tb_contact_topic_lang', $update, 'langId');
        ...
        if (!empty($insert)):
            ...
            return $this->db->insert_batch('tb_contact_topic_lang', $insert);
        endif;
